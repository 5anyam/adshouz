...
const ticker = [
  ...
  "International Ads", "Analytics & Reporting", "Landing Pages",
  "CRO", "Lead Generation",
];
...
const StatItem = ({ value, suffix, label, trigger }: { value: number; suffix: string; label: string; trigger: boolean }) => {
  ...
  return (
    <div className="text-center">
      <div className="text-4xl sm:text-5xl lg:text-6xl font-black text-white tabular-nums">
        {count}<span className="text-violet-400">{suffix}</span>
      </div>
      <div className="text-white/40 text-xs uppercase tracking-widest font-semibold mt-2">{label}</div>
    </div>
  );
};

// Infinite Marquee — duplicate items so scroll is seamless
const Marquee = () => {
  const items = [...ticker, ...ticker, ...ticker, ...ticker];
  return (
    <div className="border-y border-white/8 bg-[#0F0F14] py-3.5 overflow-hidden select-none">
      <div className="flex gap-10 animate-marquee whitespace-nowrap w-max">
        {items.map((item, i) => (
          <span
            key={i}
            className="inline-flex items-center gap-2.5 text-white/35 text-[11px] font-bold uppercase tracking-widest flex-shrink-0"
          >
...
function fmtDate(d: string) { return new Date(d).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }); }
...
export default function HomePage() {
  ...
  // Lock body when video modal open
  useEffect(() => {
    ...
    <div className="bg-[#0B0B0F] text-white overflow-x-hidden">
      ...
      <section className="relative min-h-[100svh] flex items-center pt-24 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute -top-32 -right-20 w-[500px] h-[500px] lg:w-[700px] lg:h-[700px] bg-violet-700/25 rounded-full blur-[120px]" />
          <div className="absolute bottom-0 -left-10 w-[300px] h-[300px] lg:w-[500px] lg:h-[500px] bg-purple-800/15 rounded-full blur-[100px]" />
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 w-full relative z-10">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            ...
            {/* LEFT */}
            <div
              style={{ animation: "heroFadeIn 0.9s cubic-bezier(0.22,1,0.36,1) both" }}
            >
              {/* Badge */}
              ...
              <p className="text-base sm:text-lg text-white/55 mb-8 max-w-md leading-relaxed">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                ...
                <a
                  href={`tel:${PHONE}`}
                  className="inline-flex items-center justify-center gap-2 border border-white/10 text-white font-semibold px-7 py-4 rounded-2xl hover:bg-white/5 transition-all text-sm sm:text-base"
                >
                ...
                <div className="flex -space-x-3">
                  {avatars.map((src, i) => (
                    <img key={i} src={src} alt="client"
                      className="w-9 h-9 rounded-full border-2 border-[#0B0B0F] object-cover" />
                  ))}
                </div>
                <div>
            ...
            {/* RIGHT — visible on lg; stacked mobile stats below */}
            <div className="relative hidden lg:block">
              <div className="relative rounded-3xl overflow-hidden border border-white/8">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-tr from-[#0B0B0F]/70 via-transparent to-violet-900/20" />
              </div>

              {/* Floating ROI */}
              <div className="absolute -left-10 top-10 bg-[#13131A]/90 backdrop-blur-sm border border-white/10 rounded-2xl p-5 shadow-2xl">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-violet-500/20 rounded-xl flex items-center justify-center">
                    <TrendingUp className="w-5 h-5 text-violet-400" />
                  </div>
                  <div>
                    <div className="text-2xl font-black text-white">300<span className="text-violet-400">%</span></div>
                    <div className="text-white/40 text-xs">Average ROI</div>
                  </div>
                </div>
              </div>

              {/* Floating Live */}
              <div className="absolute -right-8 bottom-12 bg-[#13131A]/90 backdrop-blur-sm border border-white/10 rounded-2xl p-5 shadow-2xl">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
                  <span className="text-white/40 text-[10px] font-bold uppercase tracking-wider">Live Campaigns</span>
                </div>
                <div className="text-3xl font-black text-white">126<span className="text-violet-400">+</span></div>
                <div className="text-white/40 text-xs mt-0.5">Active Clients</div>
              </div>
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <button
                  ...
                  className="w-16 h-16 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all shadow-xl"
                >
          ...
          <div className="grid lg:grid-cols-2 gap-14 lg:gap-20 items-center">
            ...
            <FadeUp>
              <div className="relative">
                ...
                {/* Stat overlay */}
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-[#13131A] border border-violet-500/20 rounded-2xl px-7 py-4 text-center shadow-2xl shadow-black/70 w-48 sm:w-56">
                  <div className="text-4xl sm:text-5xl font-black text-white">8<span className="text-violet-400">+</span></div>
                  <div className="text-white/45 text-xs mt-1 leading-snug">Years in Performance Marketing</div>
                </div>
              </div>
            </FadeUp>
            ...
            <FadeUp delay={150}>
              ...
              <p className="text-white/55 mb-7 leading-relaxed text-sm sm:text-base">
              ...
              <div className="space-y-3 mb-8">
                {expertise.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-violet-400 flex-shrink-0" />
      ...
      {/* ════════════════════════════ SERVICES ═══════════════════════════ */}
      <section className="py-20 lg:py-24 bg-[#0F0F14]">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          <FadeUp className="text-center mb-12 lg:mb-16">
            ...
            <p className="text-white/40 max-w-xl mx-auto text-sm mt-2 px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {services.map(({ icon: Icon, title, desc }, i) => (
              <FadeUp key={i} delay={i * 60}>
                <div className="group bg-[#13131A] border border-white/5 rounded-2xl p-5 sm:p-6 hover:border-violet-500/40 hover:bg-[#16161F] transition-all duration-300 hover:-translate-y-1.5 cursor-default h-full">
                  ...
                  <h3 className="font-bold text-white mb-2 text-[14px] sm:text-[15px]">{title}</h3>
                  <p className="text-white/40 text-xs leading-relaxed">{desc}</p>
                  <div className="mt-4 flex items-center text-violet-400 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-all">
      ...
      {/* ═════════════════════════════ STATS ═════════════════════════════ */}
      <div ref={statsSection.ref} className="py-16 lg:py-20 border-y border-white/8">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
      ...
      <section className="py-20 lg:py-28 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-violet-800/10 rounded-full blur-[100px]" />
        </div>
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {process.map(({ step, title, desc }, i) => (
              <FadeUp key={i} delay={i * 100}>
                <div className="relative flex lg:block gap-5 lg:gap-0 bg-[#13131A] border border-white/5 rounded-2xl p-5 sm:p-7 hover:border-violet-500/30 transition-all hover:-translate-y-1 duration-300 h-full">
                  {/* Mobile: left number, Desktop: top number */}
      ...
      <section className="py-20 lg:py-24 bg-[#0F0F14]">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <FadeUp delay={100} className="max-w-2xl lg:max-w-3xl mx-auto">
            <div className="bg-[#13131A] border border-white/5 rounded-3xl p-7 sm:p-10 relative overflow-hidden">
              ...
              <p
                key={activeT}
                className="text-white/80 text-base sm:text-xl leading-relaxed mb-7 italic relative z-10"
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="font-black text-white text-sm sm:text-base">{testimonials[activeT].name}</div>
                  <div className="text-violet-400 text-xs sm:text-sm mt-0.5">{testimonials[activeT].role}</div>
                </div>
                {/* Dots */}
                  ...
                  {testimonials.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all duration-300 ${
                        i === activeT ? 'bg-violet-500 w-7' : 'bg-white/15 w-1.5 hover:bg-white/30'
                      }`}
                    />
                  ))}
                </div>
          ...
          <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
            <FadeUp>
              <div className="flex items-end justify-between mb-10 lg:mb-12">
                ...
                <Link
                  href="/blogs"
                  className="hidden sm:inline-flex items-center gap-1 text-violet-400 font-semibold hover:text-violet-300 transition-colors text-sm flex-shrink-0"
                >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {posts.map((post, i) => (
                <FadeUp key={post.id} delay={i * 100}>
                  <article className="bg-[#13131A] border border-white/5 rounded-2xl overflow-hidden group hover:border-violet-500/30 hover:-translate-y-1.5 transition-all duration-300 h-full flex flex-col">
                    <div className="aspect-video overflow-hidden flex-shrink-0">
                      {post._embedded?.['wp:featuredmedia']?.[0]?.source_url ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-violet-900/30 to-purple-900/10 flex items-center justify-center">
                          <BookOpen className="w-10 h-10 text-violet-500/30" />
                        </div>
                      )}
                    </div>
                    <div className="p-5 sm:p-6 flex flex-col flex-1">
                      <div className="text-white/25 text-xs mb-3 flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5" /> {fmtDate(post.date)}
                      </div>
                      <h3 className="font-bold text-white mb-2 group-hover:text-violet-300 transition-colors line-clamp-2 text-sm flex-1">
                        <Link href={`/blogs/${post.slug}`}>{post.title.rendered}</Link>
                      </h3>
                      <p className="text-white/35 text-xs line-clamp-2 mb-4 leading-relaxed">
                        {stripHtml(post.excerpt.rendered)}
                      </p>
                      <Link
                        href={`/blogs/${post.slug}`}
                        className="inline-flex items-center text-violet-400 text-xs font-semibold hover:text-violet-300 transition-colors"
                      >
            ...
            <div className="text-center mt-8 sm:hidden">
              <Link href="/blogs" className="inline-flex items-center gap-2 border border-violet-500/30 text-violet-400 px-6 py-3 rounded-xl text-sm font-semibold hover:bg-violet-500/10 transition-all">
      ...
      <section className="py-20 lg:py-28 relative overflow-hidden">
        ...
        <FadeUp className="max-w-4xl mx-auto px-5 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-white/50 text-base sm:text-lg mb-8 max-w-xl mx-auto leading-relaxed px-2">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              href={`tel:${PHONE}`}
              className="inline-flex items-center justify-center gap-2 border border-white/15 text-white font-bold px-8 sm:px-10 py-4 sm:py-5 rounded-2xl hover:bg-white/5 transition-all text-base sm:text-lg"
            >
      ...
      {videoOpen && (
        ...
        >
          <div
            className="relative w-full max-w-3xl bg-[#13131A] rounded-2xl overflow-hidden border border-white/10"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setVideoOpen(false)}
              className="absolute top-3 right-3 z-10 w-8 h-8 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            <div className="aspect-video bg-[#0B0B0F] flex items-center justify-center">
              <p className="text-white/30 text-sm">Add your video embed here</p>
            </div>
          </div>
        </div>
      )}
